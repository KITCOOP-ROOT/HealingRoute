...
function doLogin(){
	if(loginForm.m_id.value === ""){
		...
		loginForm.action = "member_loginOk.jsp";
...
function confirmDelete() { //선택 삭제 
	if(confirm("삭제하시겠습니까?")) {
		memberForm.action = "../member/member_delete.jsp";
...
function confirmDeleteEach(id) { //개별 삭제 
	if(confirm("삭제하시겠습니까?")) {
		var link = "../member/member_delete.jsp?m_id=" + id; 
		location.href=link; 
	}
}
...
function selectCheckBoxAll(){ 
	var checkBoxes = memberForm.check; 
	for(i in checkBoxes) {
		checkBoxes[i].checked = (memberForm.allCheck.checked) ? true : false;
  }
} 
